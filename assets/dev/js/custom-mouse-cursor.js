; let haCursorOptions = null;
let haCursor = null;

function initiateHaCursorObject (speed = 0.2) {
	haCursorOptions = {
		el: null,
...
( function ( $, w ) {
	...
	$window.on( "elementor/frontend/init", function ( $e ) {
		...
		let HappyCustomMouseCursor = elementorModules.frontend.handlers.Base.extend( {
			...
			getReadySettings: function () {
				...
				// Get settings 
				let cmc_switcher = this.getElementSettings( 'ha_cmc_switcher' );
				let type = this.getElementSettings( 'ha_cmc_type' );
				let icon = this.getElementSettings( 'ha_cmc_icon' );
				let text = this.getElementSettings( 'ha_cmc_text' );
				let image = this.getElementSettings( 'ha_cmc_image' );
				let video = this.getElementSettings( 'ha_cmc_video' );

				// Assign values to settings object
				if ( cmc_switcher ) settings.cmc_switcher = cmc_switcher;
				if ( type ) settings.type = type;
				if ( icon ) settings.icon = `<i  class="${icon.value}"></i>`;
			...
			run: function () {
				...
				if ( 'yes' == settings.cmc_switcher || settings.cmc_switcher != undefined ) {
					...
					let parentElement = this.$element.parents( '.e-parent' );
					let hasText = undefined;
					let hasImg = undefined;
					let hasVideo = undefined;
					...
					if ( !( typeof hasImg != 'undefined' || typeof hasVideo != 'undefined' || typeof hasText != 'undefined' ) ) {
						...
						let uniqueID = this.$element.data( 'id' );
						let uniqueSelector = 'elementor-element-' + uniqueID;
